CATEGORIES = {
    "seo": {
        "title": "SEO и трафик",
        "code": 17,
        "subcategories": {
            "traffic": {
                "title": "Трафик",
                "code": 72,
            },
            "semantic-core": {
                "title": "Семантическое ядро",
                "code": 71,
            },
            "links": {
                "title": "Ссылки",
                "code": 59,
            },
            "statistics-analytics": {
                "title": "Статистика и аналитика",
                "code": 56,
            },
            "seo-audits": {
                "title": "SEO аудиты, консультации",
                "code": 44,
            },
            "internal-optimization": {
                "title": "Внутренняя оптимизация",
                "code": 43,
            },
            "top-promotion": {
                "title": "Продвижение сайта в топ",
                "code": 273,
            },
        }
    },
    "audio-video": {
        "title": "Аудио, видео, съемка",
        "code": 7,
        "subcategories": {
            "audio-editing": {
                "title": "Редактирование аудио",
                "code": 106,
            },
            "video-shooting": {
                "title": "Видеосъемка и монтаж",
                "code": 78,
            },
            "logo-animation": {
                "title": "Интро и анимация логотипа",
                "code": 77,
            },
            "video-clips": {
                "title": "Видеоролики",
                "code": 76,
            },
            "music-songs": {
                "title": "Музыка и песни",
                "code": 23,
            },
            "audio-recording": {
                "title": "Аудиозапись и озвучка",
                "code": 20,
            },
        }
    },
    "business-life": {
        "title": "Бизнес и жизнь",
        "code": 83,
        "subcategories": {
            "site-sale": {
                "title": "Продажа сайтов",
                "code": 114,
            },
            "personal-assistant": {
                "title": "Персональный помощник",
                "code": 84,
            },
            "accounting-taxes": {
                "title": "Бухгалтерия и налоги",
                "code": 64,
            },
            "legal-help": {
                "title": "Юридическая помощь",
                "code": 63,
            },
            "education-consulting": {
                "title": "Обучение и консалтинг",
                "code": 55,
            },
            "calls-sales": {
                "title": "Обзвоны и продажи",
                "code": 262,
            },
            "staff-selection": {
                "title": "Подбор персонала",
                "code": 265,
            },
            "construction-repair": {
                "title": "Стройка и ремонт",
                "code": 65,
            },
        }
    },
    "design": {
        "title": "Дизайн",
        "code": 15,
        "subcategories": {
            "logo-branding": {
                "title": "Логотип и брендинг",
                "code": 25,
            },
            "web-mobile-design": {
                "title": "Веб и мобильный дизайн",
                "code": 24,
            },
            "art-illustrations": {
                "title": "Арт и иллюстрации",
                "code": 28,
            },
            "printing": {
                "title": "Полиграфия",
                "code": 27,
            },
            "interior-exterior": {
                "title": "Интерьер и экстерьер",
                "code": 90,
            },
            "industrial-design": {
                "title": "Промышленный дизайн",
                "code": 250,
            },
            "presentations-infographics": {
                "title": "Презентации и инфографика",
                "code": 270,
            },
            "editing-retouching": {
                "title": "Обработка и редактирование",
                "code": 68,
            },
            "outdoor-advertising": {
                "title": "Наружная реклама",
                "code": 272,
            },
            "marketplaces-socialmedia": {
                "title": "Маркетплейсы и соцсети",
                "code": 286,
            },
        }
    },
    "programming": {
        "title": "Разработка и IT",
        "code": 11,
        "subcategories": {
            "site-repair": {
                "title": "Доработка и настройка сайта",
                "code": 38,
            },
            "site-creation": {
                "title": "Создание сайта",
                "code": 37,
            },
            "scripts": {
                "title": "Скрипты и боты",
                "code": 41,
            },
            "markup": {
                "title": "Верстка",
                "code": 79,
            },
            "desktop": {
                "title": "Десктоп программирование",
                "code": 80,
            },
            "mobile-apps": {
                "title": "Мобильные приложения",
                "code": 39,
            },
            "games": {
                "title": "Игры",
                "code": 40,
            },
            "hosting": {
                "title": "Сервера и хостинг",
                "code": 255,
            },
            "testing": {
                "title": "Юзабилити, тесты и помощь",
                "code": 81,
            },
        }
    },
    "socialmedia-advertising": {
        "title": "Соцсети и реклама",
        "code": 45,
        "subcategories": {
            "databases-clients": {
                "title": "Базы данных и клиентов",
                "code": 113,
            },
            "marketplaces-ads": {
                "title": "Маркетплейсы и доски объявлений",
                "code": 112,
            },
            "email-marketing": {
                "title": "E-mail маркетинг и рассылки",
                "code": 108,
            },
            "context-advertising": {
                "title": "Контекстная реклама",
                "code": 49,
            },
            "advertising-pr": {
                "title": "Реклама и PR",
                "code": 47,
            },
            "socialmedia-smm": {
                "title": "Соцсети и SMM",
                "code": 46,
            },
        }
    },
    "texts-translations": {
        "title": "Тексты и переводы",
        "code": 5,
        "subcategories": {
            "resumes-vacancies": {
                "title": "Резюме и вакансии",
                "code": 235,
            },
            "text-typing": {
                "title": "Набор текста",
                "code": 75,
            },
            "sales-business-texts": {
                "title": "Продающие и бизнес-тексты",
                "code": 74,
            },
            "site-texts": {
                "title": "Тексты и наполнение сайта",
                "code": 73,
            },
            "translations": {
                "title": "Переводы",
                "code": 35,
            },
        }
    }
}

CATEGORIES_REVERSE = {v["code"]: k for k, v in CATEGORIES.items()}
SUBCATEGORIES_REVERSE = {
    subcat["code"]: (cat, subcat_key)
    for cat, data in CATEGORIES.items()
    for subcat_key, subcat in data["subcategories"].items()
}
